...
export default function Revogue() {
  ...
  const handleLogin = () => {
    ...
    if (isEmail(userContact)) setUserProfile(p => ({ ...p, email: userContact }));
    else setUserProfile(p => ({ ...p, phone: userContact }));
    if (authMode === 'signin' && !userName.trim()) {
      // derive a friendly name from email/phone for sign-in
      setUserName(isEmail(userContact) ? userContact.split('@')[0] : 'You');
    }
    setUserRole('buyer'); // default role; user can switch later in profile
    setScreen('app');
  };
  ...
  const allProducts = useMemo(
    () => [...userListings, ...SHUFFLED_PRODUCTS].map(productWithImg),
    [userListings, SHUFFLED_PRODUCTS]
  );
  ...
  const renderStyleFeed = () => {
    ...
    return (
      <>
        ...
        {STYLE_POSTS.map((post, i) => {
          const liked = !!postLikes[post.id];
          const saved = !!postSaves[post.id];
          return (
        ...
        {/* Comments overlay */}
        {openComments !== null && (() => {
          const post = STYLE_POSTS.find(p => p.id === openComments);
          if (!post) return null;
          const comments = postComments[openComments] || [];
          const seedComments = [
            { user: 'thrift_thakur', text: 'Obsessed with this fit 🔥', time: '1h' },
            { user: 'silk_road', text: 'Where did you get the bag from??', time: '45m' },
          ];
          const allComments = [...seedComments, ...comments];
          const submitComment = () => {
  ...
  const renderProfile = () => {
    ...
    <>
      ...
      <div style={{padding:20}}>
        <button onClick={() => { setScreen('role'); setUserRole(null); setUserName(''); setUserContact(''); setAuthPassword(''); setAuthMode('signin'); setWishlist([]); setCart([]); setUserListings([]); setOrders([]); setUserAvatar(null); setActiveTab('home'); }} style={{width:'100%',padding:14,background:'transparent',border:'1px solid #d6cab4',borderRadius:14,fontFamily:'inherit',fontSize:12,letterSpacing:1,textTransform:'uppercase',color:'var(--ink-soft)',cursor:'pointer'}}>Sign out</button>
      </div>
    </>
    );
  };
  ...
  const renderEditProfile = () => {
    ...
    const handleAvatarUpload = async (e) => {
      ...
      try {
        ...
        setEditDraft({ ...editDraft, avatarImg: dataUrl });
    ...
    const saveProfile = () => {
      ...
      setUserAvatar(editDraft.avatarImg !== undefined ? editDraft.avatarImg : userAvatar);
      setUserProfile({
        bio: editDraft.bio,
            ...
            {addresses.map(a => (
              <div key={a.id} style={{background:'var(--paper)',border: a.isDefault ? '1.5px solid var(--terracotta)' : '1px solid #eae0cc',borderRadius:14,padding:14,marginBottom:10,position:'relative'}}>
                ...
                <div style={{display:'flex',gap:8,marginTop:10}}>
                  {!a.isDefault && <button onClick={() => setAddresses(prev => prev.map(x => ({ ...x, isDefault: x.id === a.id })))} style={{flex:1,padding:'7px 10px',background:'var(--paper)',border:'1px solid #d6cab4',borderRadius:8,fontSize:11,fontWeight:500,cursor:'pointer',fontFamily:'inherit'}}>Set as default</button>}
                  <button onClick={() => setAddresses(prev => prev.filter(x => x.id !== a.id))} style={{flex:!a.isDefault ? '0 0 auto' : 1,padding:'7px 10px',background:'var(--paper)',border:'1px solid #d6cab4',borderRadius:8,fontSize:11,fontWeight:500,cursor:'pointer',fontFamily:'inherit',color:'#c94848'}}>Delete</button>
                </div>
              </div>
            ))}
          </div>
              ...
              <div key={p.id} style={{background:'var(--paper)',border: p.isDefault ? '1.5px solid var(--terracotta)' : '1px solid #eae0cc',borderRadius:14,padding:14,marginBottom:10,display:'flex',alignItems:'center',gap:12}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{display:'flex',gap:6,marginTop:8}}>
                    {!p.isDefault && <button onClick={() => setPaymentMethods(prev => prev.map(x => ({ ...x, isDefault: x.id === p.id })))} style={{padding:'4px 10px',background:'var(--cream)',border:'1px solid #d6cab4',borderRadius:8,fontSize:10,cursor:'pointer',fontFamily:'inherit'}}>Make default</button>}
                    <button onClick={() => setPaymentMethods(prev => prev.filter(x => x.id !== p.id))} style={{padding:'4px 10px',background:'var(--cream)',border:'1px solid #d6cab4',borderRadius:8,fontSize:10,cursor:'pointer',fontFamily:'inherit',color:'#c94848'}}>Remove</button>
                  </div>
                </div>
  ...
  const renderSustainability = () => {
    const itemsBought = orders.reduce((s, o) => s + o.items.length, 0);
    const itemsSold = userListings.length;
    const totalImpact = itemsBought + itemsSold;
    // ~10kg CO2 saved per thrifted garment vs new
    const co2Saved = totalImpact * 10;
    const waterSaved = totalImpact * 2700; // ~2700L per cotton tee
    const score = Math.min(100, totalImpact * 8 + 12);
    const tier = score < 30 ? 'Sprout 🌱' : score < 60 ? 'Bloom 🌿' : score < 85 ? 'Grove 🌳' : 'Forest 🌲';
    return (
      <>
  ...
  const renderSettings = () => {
    ...
    const setKey = (k) => setSettings({ ...settings, [k]: !settings[k] });
    ...
    return (
      <>
        ...
        <div className="rv-menu-item" onClick={() => { const next = userRole === 'seller' ? 'buyer' : 'seller'; setUserRole(next); pushToast(`Switched to ${next} mode`, 'success'); }}>
  ...
  const renderSell = () => {
    ...
    const handleListingPhotos = async (e) => {
      ...
      try {
        const urls = await Promise.all(toAdd.map(f => {
          if (!f.type.startsWith('image/')) return null;
          return fileToDataUrl(f);
        }));
        setDraft({ imgs: [...draft.imgs, ...urls.filter(Boolean)] });
  ...
  const renderPostStyle = () => (
    <>
      ...
      <div className="rv-field">
        ...
        <textarea className="rv-input" placeholder="What's the story behind this fit?" rows={3} style={{resize:'none',fontFamily:'inherit'}}/>
      </div>
      <div className="rv-field">
        <div className="rv-label">Tag pieces from Revogue</div>
        <button style={{width:'100%',padding:14,background:'var(--cream)',border:'1px dashed #d6cab4',borderRadius:14,fontFamily:'inherit',fontSize:13,color:'var(--ink-soft)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
          <Plus size={14}/> Link from your closet or wishlist
        </button>
      </div>
      <div style={{padding:'10px 20px 30px'}}>
        <button className="rv-btn-primary" onClick={() => { alert('Posted to Lookbook! ✨'); setActiveTab('style'); }}>Post to Lookbook</button>
      </div>
    </>
  );
  ...
  const renderPayment = () => (
    <>
      ...
      <div style={{padding:'10px 20px 30px'}}>
        <button className="rv-btn-primary" onClick={() => {
          ...
          pushToast(`Order ${orderNum} placed`, 'success');
      ...
      <>
        ...
        <div className="rv-detail-body">
          ...
          <div className="rv-seller-card">
            <div className="rv-seller-avatar">{p.seller[0].toUpperCase()}</div>
  ...
  else if (activeTab === 'sell') content = userRole === 'seller' ? renderSell() : renderSell();
